...
public class ArticleWithCommentsResponseTest {
    ...
    void givenArticleWithCommentsDtoWithoutChildComments_whenMapping_thenOrganizesCommentsWithCertainOrder() {
        ...
        //댓글모음 Set을 생성했다. 모두 부모댓글이 없는 댓글들이다.
        Set<ArticleCommentDto> articleCommentDtos = Set.of(
            createArticleCommentDto(1L, null, now),  //부모댓글이 없는 케이스
        ...
        // 게시글을 댓글과 함께 생성된 dto로 생성.
        ArticleWithCommentsDto input = createArticleWithCommentsDto(articleCommentDtos);

        //When : 댓글이 달려있는 게시물 Dto를 ArticleWithCommentResponse 객체로 변환(from함수)
        ArticleWithCommentsResponse actual = ArticleWithCommentsResponse.from(input);

        //Then : ArticleWithCommentsResponse 객체 안에는 최초에 설정해놓은 댓글 데이터가 그대로 있어야 한다는 것을 검증.
    ...
    @DisplayName("게시글 + 댓글 dto를 api 응답으로 변환할 때, 부모 자식 관계 깊이(depth)에는 제한이 없다.")
    @Test
